...
- [x] **IMP-02**: User sees the waveform of the loaded file
- [x] **IMP-03**: User can select a region on the waveform for analysis, or analyze the whole file (default)
...
- [x] **ANL-04**: Analysis runs on a background thread — UI stays responsive, progress is shown; a 3-minute song analyzes in seconds
- [x] **ANL-05**: Detected chords are displayed as named chords (Am, Cmaj7, F/A) on a timeline over the waveform
...
- [x] **GEN-10**: 5 main-genre chips displayed near a narrower waveform strip; a menu lists ALL genres with checkboxes where exactly 5 can be picked as main
...
- [x] **EXP-01**: User can drag any MIDI row from the plugin straight into the DAW piano roll (temp .mid + external file drag)
- [x] **EXP-02**: User can save any MIDI row to disk as a .mid file (fallback path)
...
| GEN-10 | Phase 6.1 - Genre Patterns & Premium Conveyor | Complete |
