...
namespace
{
    ...
    // <interfaces>). Slot-stable colour mapping (06.1-01-PLAN.md Task 1 step
    // 7): colour follows slot (PatternKind), not genre, so a row's visual
    // identity stays consistent across a genre switch.
    juce::Colour accentForKind (PatternKind kind)
    {
        switch (kind)
        {
            case PatternKind::SustainedChords: return juce::Colour (0xffd8d8e0);
            case PatternKind::RhythmicChords:  return juce::Colour (0xff3a4a6e);
            case PatternKind::StabArp:         return juce::Colour (0xff6e5a2f);
            case PatternKind::BassLine:        return juce::Colour (0xff7ec850);
            case PatternKind::TopLineMotif:    return juce::Colour (0xffe8c547);
        }
...
void MidiRowView::paint (juce::Graphics& g)
{
    ...
    auto accent = accentForKind (row.kind);
